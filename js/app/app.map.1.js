app.controller("MapController", function(store, actions){
    ...
    var editPoints = new L.FeatureGroup();
    var editEdges = new L.FeatureGroup();
    var drawnPoints = new L.FeatureGroup();
    var drawnEdges = new L.FeatureGroup();
    ...
    var edgeDrawer = new L.Draw.Polyline(map, {
        repeatMode: true
    })

    store.on('ui.graph_editing_mode', function(){
        switch(store.state.ui.graph_editing_mode){
            case POINT_ADDING:
    ...
    store.on('ui.map_polygon', selfcheck(function(){
        editItems.clearLayers();
        if(!store.state.ui.map_polygon) {
            ...
            var layer = L.geoJSON(store.state.ui.map_polygon).getLayers()[0];
    ...
    map.on(L.Draw.Event.CREATED, function (e) {
        ...
        switch(geometry.type) {
            case "Polygon":
                return store.dispatch(actions.mapGeoJSON(geometry));
            case "Point":
